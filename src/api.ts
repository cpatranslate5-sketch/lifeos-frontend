...
export async function sendMessage(content: string, today: string, conversationId = "default", space = "life"): Promise<Message[]> {
  const res = await req("/messages", {
    method: "POST",
    body: JSON.stringify({ content, conversation_id: conversationId, today, space }),
  });
  return res.json();
}

export async function fetchEntities(type?: string, space?: string): Promise<Entity[]> {
...
export async function createEntity(type: string, name: string, attributes: Record<string, any> = {}, space = "life"): Promise<Entity> {
  const res = await req("/entities", { method: "POST", body: JSON.stringify({ type, name, attributes, space }) });
  return res.json();
}
