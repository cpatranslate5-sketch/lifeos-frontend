...
export async function uploadEntityCover(id: string, file: File): Promise<Entity> {
  const form = new FormData();
  form.append("file", file);
  const res = await fetch(`${API_URL}/entities/${id}/cover`, { method: "POST", headers: { ...authHeaders() }, body: form });
  if (!res.ok) throw new Error(`cover upload failed: ${res.status}`);
  return res.json();
}

export function entityCoverUrl(filename: string): string {
  return `${API_URL}/entities/cover/${filename}`;
}

export async function uploadEntityBackground(id: string, file: File): Promise<Entity> {
  const form = new FormData();
  form.append("file", file);
  const res = await fetch(`${API_URL}/entities/${id}/background`, { method: "POST", headers: { ...authHeaders() }, body: form });
  if (!res.ok) throw new Error(`background upload failed: ${res.status}`);
  return res.json();
}

export async function removeEntityBackground(id: string): Promise<Entity> {
  const res = await req(`/entities/${id}/background`, { method: "DELETE" });
  return res.json();
}

export async function applyBackgroundScope(id: string, scope: "all" | "pool"): Promise<{ updated: number }> {
  const res = await req(`/entities/${id}/background-scope`, { method: "POST", body: JSON.stringify({ scope }) });
  return res.json();
}

export async function removeBackgroundScope(id: string, scope: "same_image" | "all"): Promise<{ updated: number }> {
  const res = await req(`/entities/${id}/background-remove-scope`, { method: "POST", body: JSON.stringify({ scope }) });
  return res.json();
}

export async function propagateCover(sourceEntityId: string, keyword: string): Promise<{ updated: number }> {
  const res = await req("/entities/propagate-cover", { method: "POST", body: JSON.stringify({ source_entity_id: sourceEntityId, keyword }) });
  return res.json();
}

export async function markSport(keyword: string, profile: string, space = "life"): Promise<{ updated: number }> {
  const res = await req("/entities/mark-sport", { method: "POST", body: JSON.stringify({ keyword, profile, space }) });
  return res.json();
}
