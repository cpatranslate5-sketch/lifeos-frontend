...
function computeNotices(tasks: Entity[]): Notice[] {
  ...
  for (const t of tasks) {
    ...
    if (date < today) {
      notices.push({ id: t.id, text: `Не закрыта задача «${t.name}» — была на ${date}`, date, space: t.space });
    } else if (date === today && t.attributes?.time && t.attributes.time < nowHM) {
      notices.push({ id: t.id, text: `Просрочена задача «${t.name}» — время было ${t.attributes.time}`, date, space: t.space });
    }
  }
  return notices;
}

export default function NotificationBell({ profile, onNavigate, refreshTrigger }: { profile: string; onNavigate: (date: string, space: string) => void; refreshTrigger?: number }) {
      ...
      {open && (
        <>
          ...
          <div className="bell-dropdown" onClick={(ev) => ev.stopPropagation()}>
            ...
            {notices.map(n => (
              <div key={n.id} className="field bell-notice" onClick={() => goTo(n)}>
                <span>{n.text}</span>
                <span className="bell-notice-arrow">→</span>
              </div>
            ))}
          </div>
        </>
