...
export default function NotificationBell({ profile, onNavigate }: { profile: string; onNavigate: (date: string, space: string) => void }) {
      ...
      {open && (
        <>
          ...
          <div className="bell-dropdown" onClick={(ev) => ev.stopPropagation()}>
            ...
            {notices.map(n => (
              <div key={n.id} className="field bell-notice" style={{ marginBottom: 6, cursor: "pointer" }} onClick={() => goTo(n)}>
                {n.text}
              </div>
            ))}
          </div>
