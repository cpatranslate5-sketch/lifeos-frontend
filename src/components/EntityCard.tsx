...
export default function EntityCard({ e, onChanged, selectedDate, showNextStep, profile, onFilterByCriterion }: Props) {
  ...
  const moveReferenceDate = isHabit ? (selectedDate || todayStr()) : (e.attributes?.date || todayStr());
  const moveMinDate = addDaysStr(moveReferenceDate, 1);

  async function moveTo(targetDate: string) {
    if (isHabit && selectedDate) {
      // Skip just this occurrence, and spin off a one-off reminder on the new date
      // instead — the recurring pattern itself is untouched.
  ...
  return (
    <div className="card card-with-cover">
      ...
      {(e.type === "task" || isHabit) && !e.attributes?.done && (
      ...
      {(isHabit || e.type === "event" || e.type === "task") && (
        <div className="field-row">
          {showTime ? (
            <span className="fixed-time-field">
          ...
          ) : <span className="add-field-link" onClick={() => setShowTime(true)}>+ Время</span>}
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginTop: 4, flexWrap: "wrap" }}>
        {!["movie", "show", "book", "leisure", "game"].includes(e.type) && e.attributes?.status !== "skipped" && (
          <div className="why" style={{ color: "var(--event)" }} onClick={skip}>{isHabit ? `Отменить на ${selectedDate}` : "Пропустить/отменить"}</div>
        )}
          ...
          <div className="why" style={{ position: "relative" }} onClick={() => setMovePickerOpen(!movePickerOpen)}>
            ...
            {movePickerOpen && (
              <>
                ...
                <div className="genre-picker" onClick={(ev) => ev.stopPropagation()}>
                  <input type="date" autoFocus min={moveMinDate} defaultValue={moveMinDate}
                    onChange={(ev) => moveTo(ev.target.value)}
                    style={{ background: "var(--bg)", border: "1px solid var(--line)", borderRadius: 8, color: "var(--text)", padding: 6 }} />
                </div>
              </>
            )}
          </div>
