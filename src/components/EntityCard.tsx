...
import { Entity, updateEntityField, renameEntity, deleteEntity, createEntity, uploadEntityCover, entityCoverUrl, propagateCover, uploadEntityBackground, removeEntityBackground, applyBackgroundScope, removeBackgroundScope } from "../api";
...
export default function EntityCard({ e, onChanged, selectedDate, showNextStep, profile, onFilterByCriterion }: Props) {
  ...
  const [bgScopePrompt, setBgScopePrompt] = useState(false);
  const [removeScopePrompt, setRemoveScopePrompt] = useState(false);
  const [pasteMode, setPasteMode] = useState(false);
  const [accentColor, setAccentColor] = useState<string | null>(null);

  useEffect(() => {
    const bg = e.attributes?.background_path;
    if (!bg) { setAccentColor(null); return; }
    const img = new Image();
    ...
    img.onload = () => {
      try {
        ...
        r /= n; g /= n; b /= n;
        // Pick a contrasting accent based on which channel dominates the average.
        if (r > g && r > b) setAccentColor("#4A9EFF");       // reddish/warm bg → blue accent
        else if (b > r && b > g) setAccentColor("#E8963D");  // bluish bg → orange accent
        else if (g > r && g > b) setAccentColor("#D4527E");  // greenish bg → pink accent
        else setAccentColor("#4A9EFF");
      } catch { setAccentColor(null); }
    };
    img.onerror = () => setAccentColor(null);
    img.src = entityCoverUrl(bg);
  }, [e.attributes?.background_path]);
  ...
  async function uploadBackground(file: File | null | undefined) {
    if (!file) return;
    await uploadEntityBackground(e.id, file);
    onChanged();
    setPasteMode(false);
    setBgScopePrompt(true);
  }

  function handleCardClick(ev: React.MouseEvent) {
    if (ev.target === ev.currentTarget) setPasteMode(p => !p);
  }

  function handleCardPaste(ev: React.ClipboardEvent) {
    const items = ev.clipboardData?.items;
    if (!items) return;
    for (let i = 0; i < items.length; i++) {
      if (items[i].type.startsWith("image/")) {
        ev.preventDefault();
        uploadBackground(items[i].getAsFile());
        return;
      }
    }
  }

  async function chooseBgScope(scope: "only" | "all" | "pool") {
    setBgScopePrompt(false);
    if (scope === "only") return;
    const res = await applyBackgroundScope(e.id, scope === "all" ? "all" : "pool");
    showToast(`Применено к ${res.updated} карточкам`);
    onChanged();
  }

  function clearBackground(ev: React.MouseEvent) {
    ev.stopPropagation();
    setRemoveScopePrompt(true);
  }

  async function chooseRemoveScope(scope: "here" | "same_image" | "all" | "cancel") {
    setRemoveScopePrompt(false);
    if (scope === "cancel") return;
    if (scope === "here") {
      await removeEntityBackground(e.id);
    } else {
      const res = await removeBackgroundScope(e.id, scope);
      showToast(`Убрано у ${res.updated} карточек`);
    }
    onChanged();
  }

  async function cancelBackgroundAdd() {
    setBgScopePrompt(false);
    await removeEntityBackground(e.id);
    onChanged();
  }
  ...
  const cardBgStyle: React.CSSProperties = e.attributes?.background_path ? {
    backgroundImage: `linear-gradient(rgba(20,23,31,0.18), rgba(20,23,31,0.18)), url(${entityCoverUrl(e.attributes.background_path)})`,
  ...
  if (e.type === "anniversary") {
    return (
      <div className={`card card-with-cover ${pasteMode ? "paste-mode" : ""}`} style={cardBgStyle} tabIndex={0} onPaste={handleCardPaste} onClick={handleCardClick}
        title="Кликните на карточку и вставьте (Ctrl+V) картинку, чтобы задать фон">
        {e.attributes?.background_path && (
          <div className="card-bg-clear" onClick={clearBackground} title="Убрать фон">×</div>
        )}
        <div className="cover-thumb" tabIndex={0} onPaste={handleCoverPaste}
          onDoubleClick={() => e.attributes?.cover_path && setCoverLightbox(true)}
        ...
        <div className={`card-body ${e.attributes?.background_path ? "has-bg-fields" : ""}`}>
        ...
        {bgScopePrompt && (
          <div className="modal-bg" onClick={cancelBackgroundAdd}>
            <div className="modal bg-scope-modal" onClick={ev => ev.stopPropagation()}>
              <div style={{ marginBottom: 12 }}>Добавить фон к уже существующим карточкам?</div>
              <button onClick={() => chooseBgScope("only")}>Применить только к этой</button>
              <button onClick={() => chooseBgScope("all")}>Применить ко всем</button>
              <button onClick={() => chooseBgScope("pool")}>Добавить к имеющимся</button>
              <button className="cancel" onClick={cancelBackgroundAdd}>Отмена</button>
            </div>
          </div>
        )}
        {removeScopePrompt && (
          <div className="modal-bg" onClick={() => chooseRemoveScope("cancel")}>
            <div className="modal bg-scope-modal" onClick={ev => ev.stopPropagation()}>
              <div style={{ marginBottom: 12 }}>Удалить все фоны?</div>
              <button onClick={() => chooseRemoveScope("same_image")}>Только этот</button>
              <button onClick={() => chooseRemoveScope("here")}>Только в этой карточке</button>
              <button onClick={() => chooseRemoveScope("all")}>Все</button>
              <button className="cancel" onClick={() => chooseRemoveScope("cancel")}>Отмена</button>
            </div>
          </div>
        )}
        {confirmState && (
  ...
  return (
    <div className={`card card-with-cover ${pasteMode ? "paste-mode" : ""}`} style={cardBgStyle} tabIndex={0} onPaste={handleCardPaste} onClick={handleCardClick}
      ...
      {!isHabit && ["task", "event"].includes(e.type) && (
        <div className="why link-accent" onClick={moveToTomorrow}>Перенести на завтра</div>
      )}
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginTop: 4, flexWrap: "wrap" }}>
        {profile && ["movie", "show", "book", "game", "leisure"].includes(e.type) && (
          <div className="why link-accent" onClick={copyToOtherFolder}>Скопировать в {profile === "kotyonok" ? "НеМаленький" : "Котёнок"}</div>
        )}
        <div className="why" style={{ color: "var(--event)" }} onClick={handleDelete}>Удалить</div>
      </div>
      {bgScopePrompt && (
