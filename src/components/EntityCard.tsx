...
import { Entity, updateEntityField, renameEntity, deleteEntity, createEntity, uploadEntityCover, entityCoverUrl, propagateCover, uploadEntityBackground, removeEntityBackground } from "../api";
...
export default function EntityCard({ e, onChanged, selectedDate, showNextStep, profile, onFilterByCriterion }: Props) {
  ...
  const cardBgStyle: React.CSSProperties = e.attributes?.background_path ? {
    backgroundImage: `linear-gradient(rgba(20,23,31,0.78), rgba(20,23,31,0.78)), url(${entityCoverUrl(e.attributes.background_path)})`,
  ...
  if (e.type === "anniversary") {
    return (
      <div className="card card-with-cover" style={cardBgStyle}>
        ...
        <div className="card-body">
          ...
          <input ref={bgInputRef} type="file" accept="image/*" style={{ display: "none" }}
            onChange={(ev) => uploadBackground(ev.target.files?.[0])} />
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            <div className="why" onClick={() => bgInputRef.current?.click()}>{e.attributes?.background_path ? "Сменить фон" : "Задать фон"}</div>
            {e.attributes?.background_path && <div className="why" onClick={clearBackground}>Убрать фон</div>}
