...
import { Entity, updateEntityField, renameEntity, deleteEntity, createEntity, uploadEntityCover, entityCoverUrl } from "../api";
    ...
    <div className="card card-with-cover">
      ...
      {e.type === "game" && (
        <div className="field media-criterion" onClick={() => !developerEditing && setDeveloperEditing(true)}>
          ...
          ) : <><strong>Разработчик:</strong> {e.attributes?.developer || "не указан"}</>}
        </div>
      )}
