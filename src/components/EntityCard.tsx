...
export default function EntityCard({ e, onChanged, selectedDate, showNextStep }: Props) {
  ...
  return (
    <div className="card">
      ...
      {isHabit && <div className="field">{e.attributes?.daily ? "Ежедневно" : `День недели: ${["Пн","Вт","Ср","Чт","Пт","Сб","Вс"][Number(e.attributes?.weekday)] ?? "не задан"}`}{e.attributes?.recurring ? "" : " (не отмечена как повторяющаяся!)"}</div>}
