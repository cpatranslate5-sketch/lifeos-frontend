...
import { Entity, updateEntityField, renameEntity, deleteEntity, createEntity, uploadEntityCover, entityCoverUrl, propagateCover, uploadEntityBackground, removeEntityBackground, applyBackgroundScope } from "../api";
...
export default function EntityCard({ e, onChanged, selectedDate, showNextStep, profile, onFilterByCriterion }: Props) {
  ...
  async function clearBackground(ev: React.MouseEvent) {
    ev.stopPropagation();
  ...
  if (e.type === "anniversary") {
    return (
      <div className={`card card-with-cover ${pasteMode ? "paste-mode" : ""}`} style={cardBgStyle} tabIndex={0} onPaste={handleCardPaste} onClick={handleCardClick}
        ...
        {bgScopePrompt && (
          <div className="modal-bg" onClick={() => chooseBgScope("only")}>
