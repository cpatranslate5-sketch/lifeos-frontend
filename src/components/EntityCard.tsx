  ...
  return (
    <div className="card">
      <div className="card-top">
        <div className="type-badge" style={{ background: meta.color }}>
          <select className="emoji-select" value={e.attributes?.emoji || meta.emoji}
            onChange={async (ev) => { await updateEntityField(e.id, "emoji", ev.target.value); onChanged(); }}>
            {ALL_EMOJI.map(em => <option key={em} value={em}>{em}</option>)}
          </select>
        </div>
        {canCheck && (
          <input type="checkbox" className="done-check" checked={isDoneToday} onChange={toggleDone} />
        )}
        <input className="card-title-input" style={isDoneToday ? { textDecoration: "line-through", color: "var(--muted)" } : {}}
          defaultValue={e.name}
      ...
      ) : <div className="add-field-link" onClick={() => setShowNote(true)}>+ комментарий</div>}
