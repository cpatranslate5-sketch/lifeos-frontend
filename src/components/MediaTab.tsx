...
const DONE_LABEL: Record<string, [string, string]> = {
  movie: ["Просмотренное", "Непросмотренное"], show: ["Просмотренное", "Непросмотренное"],
  book: ["Прочитанное", "Непрочитанное"],
};
...
export default function MediaTab({ title, placeholder, type, items, onChanged, profile }: Props) {
  ...
  const [doneFilter, setDoneFilter] = useState<"" | "done" | "not_done">("");
  const [openDropdown, setOpenDropdown] = useState<"genre" | "geo" | "author" | "actor" | "rating" | null>(null);
  const [page, setPage] = useState(1);
  const isCast = type === "movie" || type === "show";
  const [doneLabelDone, doneLabelNotDone] = DONE_LABEL[type] || ["Просмотренное", "Непросмотренное"];

  useEffect(() => { setPage(1); }, [searchTerm, genreFilter, geoFilter, authorFilter, actorFilter, ratingFilter, doneFilter]);
  ...
  function handleFilterByCriterion(kind: FilterKind, value: string) {
    setSearchTerm(""); setGenreFilter([]); setGeoFilter([]); setAuthorFilter([]); setActorFilter([]); setRatingFilter([]); setDoneFilter("");
  ...
  const filtered = items.filter(e => {
    ...
    if (doneFilter === "done" && !e.attributes?.done) return false;
    if (doneFilter === "not_done" && e.attributes?.done) return false;
    return true;
  });

  const sorted = [...filtered].sort((a, b) => sortKey(a) - sortKey(b));
  const anyFilterActive = !!searchTerm || genreFilter.length > 0 || geoFilter.length > 0 || authorFilter.length > 0 || actorFilter.length > 0 || ratingFilter.length > 0 || !!doneFilter;
      ...
      <div className="media-filters">
        ...
        <div style={{ position: "relative" }}>
          ...
          {openDropdown === "rating" && (
            <>
              <div className="picker-overlay" onClick={() => setOpenDropdown(null)} />
              <div className="media-filter-dropdown" style={{ width: 200 }}>
                <div className="rating-filter-row" style={{ marginBottom: 4 }}>
                  {[10, 9, 8, 7, 6].map(n => (
                    <span key={n} className={`rating-star-btn ${ratingFilter.includes(n) ? "on" : ""}`} title={String(n)}
                      onClick={() => setRatingFilter(toggleInList(ratingFilter.map(String), String(n)).map(Number))}>★</span>
                  ))}
                </div>
                <div className="rating-filter-row" style={{ marginBottom: 8 }}>
                  {[5, 4, 3, 2, 1].map(n => (
                    <span key={n} className={`rating-star-btn ${ratingFilter.includes(n) ? "on" : ""}`} title={String(n)}
                      onClick={() => setRatingFilter(toggleInList(ratingFilter.map(String), String(n)).map(Number))}>★</span>
                  ))}
        ...
        <div style={{ position: "relative" }}>
          <div className={`media-filter-btn ${doneFilter ? "active" : ""}`} onClick={() => setDoneFilter(doneFilter === "" ? "done" : doneFilter === "done" ? "not_done" : "")}>
            {doneFilter === "done" ? doneLabelDone : doneFilter === "not_done" ? doneLabelNotDone : "Просмотрено?"}
          </div>
        </div>

        {anyFilterActive && (
          <div className="media-filter-btn" onClick={() => { setSearchTerm(""); setGenreFilter([]); setGeoFilter([]); setAuthorFilter([]); setActorFilter([]); setRatingFilter([]); setDoneFilter(""); }} style={{ color: "var(--event)" }}>
            Сбросить все
          </div>
        )}
      </div>
