...
export default function MediaTab({ title, placeholder, type, items, onChanged, profile }: Props) {
  ...
  const [openDropdown, setOpenDropdown] = useState<"genre" | "geo" | "author" | "actor" | "rating" | "status" | null>(null);
  const [page, setPage] = useState(1);
  const isCast = type === "movie" || type === "show";
  const [doneLabelDone, doneLabelNotDone] = DONE_LABEL[type] || ["Просмотрено", "Непросмотрено"];

  useEffect(() => { setPage(1); }, [searchTerm, genreFilter, geoFilter, authorFilter, actorFilter, ratingFilter, statusFilter]);
  ...
  function handleFilterByCriterion(kind: FilterKind, value: string) {
    setSearchTerm(""); setGenreFilter([]); setGeoFilter([]); setAuthorFilter([]); setActorFilter([]); setRatingFilter([]); setStatusFilter([]);
  ...
  const anyFilterActive = !!searchTerm || genreFilter.length > 0 || geoFilter.length > 0 || authorFilter.length > 0 || actorFilter.length > 0 || ratingFilter.length > 0 || statusFilter.length > 0;
  ...
  return (
    <div className="view">
      ...
      <div className="media-filters">
        ...
        {anyFilterActive && (
          <div className="media-filter-btn" onClick={() => { setSearchTerm(""); setGenreFilter([]); setGeoFilter([]); setAuthorFilter([]); setActorFilter([]); setRatingFilter([]); setStatusFilter([]); }} style={{ color: "var(--event)" }}>
            Сбросить все
          </div>
        )}
      </div>
