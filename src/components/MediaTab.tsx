...
export default function MediaTab({ title, placeholder, type, items, onChanged, profile }: Props) {
  ...
  async function submit() {
    ...
    if (newCoverFile) {
      await uploadEntityCover(created.id, newCoverFile);
    }
    setVal(""); setNewAuthor(""); setNewGeo(""); setNewGenres([]); setNewActors(""); setNewInProgress(false);
    setNewCoverFile(null); setNewCoverPreview(null); setShowDetails(false);
    onChanged();
  }
  ...
  return (
    <div className="view">
      ...
      {showDetails && (
        <div className="card" style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 10, alignItems: "flex-start", marginBottom: 10 }}>
            <div className="cover-thumb" tabIndex={0} onPaste={handleNewCoverPaste}
              title="Нажмите сюда и вставьте (Ctrl+V) картинку">
              <input ref={newCoverInputRef} type="file" accept="image/*" style={{ display: "none" }}
                onChange={(ev) => pickNewCover(ev.target.files?.[0])} />
              {newCoverPreview ? (
                <img src={newCoverPreview} alt="" />
              ) : (
            ...
            <div className="muted" style={{ fontSize: "0.72rem", paddingTop: 4 }}>Обложка — клик и Ctrl+V, или значок 📁</div>
          </div>
          ...
          <label className={`status-chip ${newInProgress ? "on" : ""}`} style={{ marginBottom: 10, display: "inline-flex" }}>
      ...
      <div className="edit-link" style={{ marginBottom: 10, display: "inline-block" }} onClick={() => setShowStats(!showStats)}>
        {showStats ? "скрыть статистику" : "📊 статистика"}
      </div>

      {showStats && (
        <div className="card" style={{ marginBottom: 14 }}>
          <div className="muted" style={{ marginBottom: 8 }}>По {items.length} {items.length === 1 ? "записи" : "записям"}</div>
          ...
          {topCounts(items.flatMap(e => authorsOf(e)).filter(a => a !== "Без автора")).map(([a, n]) => (
            <div key={a} className="field">{a} — {n}</div>
          ))}
          {items.every(e => authorsOf(e).includes("Без автора") && authorsOf(e).length === 1) && <div className="muted" style={{ marginBottom: 8 }}>нет данных</div>}
