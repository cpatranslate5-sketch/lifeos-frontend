...
export default function MediaTab({ title, placeholder, type, items, onChanged, profile }: Props) {
  ...
  const anyFilterActive = genreFilter.length > 0 || !!geoFilter || !!authorFilter;
  ...
  return (
    <div className="view">
      ...
      <div className="media-filters">
        ...
        {anyFilterActive && (
          <div className="media-filter-btn" onClick={() => { setGenreFilter([]); setGeoFilter(""); setAuthorFilter(""); }} style={{ color: "var(--event)" }}>
            сбросить все
          </div>
        )}
      </div>
