...
export default function ShelfTab({ title, placeholder, type, items, onChanged, extraAttrs, space, profile }: Props) {
  ...
  const [openDropdown, setOpenDropdown] = useState<"rating" | "status" | "developer" | null>(null);
  ...
  const anyFilterActive = ratingFilter.length > 0 || statusFilter.length > 0 || developerFilter.length > 0;
  const sorted = [...filtered].sort((a, b) => sortKey(a) - sortKey(b));
  const totalPages = Math.max(1, Math.ceil(sorted.length / PAGE_SIZE));
  const pageItems = sorted.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);

  useEffect(() => { setPage(1); }, [items.length, ratingFilter, statusFilter, developerFilter]);

  function handleFilterByCriterion(kind: FilterKind, value: string) {
    if (kind === "author") { setRatingFilter([]); setStatusFilter([]); setDeveloperFilter([value]); }
  }
  ...
  return (
    <div className="view">
      ...
      {isGame ? (
        <>
          ...
          <div className="media-filters">
            ...
            {anyFilterActive && (
              <div className="media-filter-btn" onClick={() => { setRatingFilter([]); setStatusFilter([]); setDeveloperFilter([]); }} style={{ color: "var(--event)" }}>
                Сбросить все
              </div>
            )}
          </div>
      ...
      {pageItems.map(it => <EntityCard key={it.id} e={it} onChanged={onChanged} profile={profile} onFilterByCriterion={isGame ? handleFilterByCriterion : undefined} />)}
