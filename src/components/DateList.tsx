...
export default function DateList({ items, selectedDate, onChanged }: { items: Entity[]; selectedDate: string; onChanged: () => void }) {
  ...
  const hasAnyOrder = sorted.some(e => typeof e.attributes?.order === "number");

  async function resetOrder() {
    if (!window.confirm("Сбросить ручной порядок для всех карточек этого списка? Сортировка вернётся к времени/пометкам (утро/день/вечер).")) return;
    await Promise.all(sorted
      .filter(e => typeof e.attributes?.order === "number")
      .map(e => updateEntityField(e.id, "order", null)));
    onChanged();
  }
  ...
  return (
    <>
      {hasAnyOrder && (
        <div className="why" style={{ marginBottom: 8, display: "inline-block" }} onClick={resetOrder}>
          сбросить ручной порядок и вернуть сортировку по времени/пометкам
        </div>
      )}
      {sorted.map(item => (
