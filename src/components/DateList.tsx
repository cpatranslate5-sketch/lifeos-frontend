...
// МЕТКА ДЛЯ ПРОВЕРКИ: sort-fix-v2 — если видите эту строку на GitHub, файл обновился
function sortItems(items: Entity[]): Entity[] {
  return [...items].sort((a, b) => {
    const aDone = !!a.attributes?.done;
    const bDone = !!b.attributes?.done;
    ...
    <div>
      ...
      {sorted.map(e => (
        <div key={e.id}
          ...
          <EntityCard e={e} onChanged={onChanged} selectedDate={selectedDate} />
        </div>
      ))}
    </div>
  );
