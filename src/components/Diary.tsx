...
import { DiaryEntry, fetchDiaryEntries, createDiaryEntry, deleteDiaryEntry, diaryPhotoUrl } from "../api";
      ...
      <div className="view">
        ...
        {entries.map(e => (
          <div key={e.id} className="card" style={{ marginBottom: 10 }}>
            ...
            <div className="why" style={{ color: "var(--event)" }} onClick={() => remove(e.id)}>удалить</div>
          </div>
        ))}
      </div>
    </div>
