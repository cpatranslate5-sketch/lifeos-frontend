...
export default function Diary({ onSwitchFolder }: { onSwitchFolder: () => void }) {
  ...
  const [lastYearEntries, setLastYearEntries] = useState<DiaryEntry[]>([]);
  ...
  async function load() {
    ...
    try {
      if (viewMode === "day") {
        ...
        setLastYearEntries(await fetchDiaryEntries(lastYearDate, person));
      } else {
  ...
  return (
    <div className="app">
      ...
      <div className="view">
        ...
        {viewMode === "day" && lastYearEntries.length > 0 && (
          <>
            <div className="diary-date-label">Год назад в этот день</div>
            {lastYearEntries.map(e => (
              <div key={e.id} className="card" style={{ marginBottom: 10, opacity: 0.85 }}>
                <div style={{ fontSize: "0.9rem", marginBottom: e.photo_paths.length ? 8 : 0 }}>{e.text}</div>
                {e.photo_paths.length > 0 && (
