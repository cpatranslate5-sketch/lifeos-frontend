...
const WORK_TABS = [
  ["chat", "Чат"], ["yesterday", "Вчера"], ["today", "Сегодня"], ["tomorrow", "Завтра"],
  ["date", "Дата"], ["longterm", "Долгосрочное"],
] as const;
...
function itemsForDate(entities: Entity[], dateStr: string, isToday: boolean): Entity[] {
  ...
  return entities.filter(e => {
    ...
    if (e.type === "habit") {
      return e.attributes?.recurring && Number(e.attributes?.weekday) === wd &&
...
export default function App() {
  ...
  const [space, setSpace] = useState<"life" | "work">("life");
  ...
  const TABS = space === "life" ? LIFE_TABS : WORK_TABS;

  async function refresh() {
    const [ents, lp] = await Promise.all([
      fetchEntities(undefined, space),
  ...
  function switchSpace(next: "life" | "work") {
    setSpace(next);
    setTab("chat");
  }
    ...
    <div className="app">
      ...
      {tab === "yesterday" && (
        <div className="view">
          ...
          <QuickTaskAdd date={yesterdayValue} onAdded={refresh} space={space} />
      ...
      {tab === "today" && (
        <div className="view">
          ...
          <QuickTaskAdd date={todayValue} onAdded={refresh} space={space} />
      ...
      {tab === "tomorrow" && (
        <div className="view">
          ...
          <QuickTaskAdd date={tomorrowValue} onAdded={refresh} space={space} />
      ...
      {tab === "date" && (
        <div className="view">
          ...
          <QuickTaskAdd date={selectedDate} onAdded={refresh} space={space} />
