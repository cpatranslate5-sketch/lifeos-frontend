...
const LIFE_TABS = [
  ["chat", "Чат"], ["date", "Дата"], ["dates", "События"], ["sport", "Спорт"], ["movies", "Кино"], ["shows", "Сериалы"], ["books", "Книги"],
  ["games", "Игры"], ["leisure", "Досуг"], ["household", "Быт"], ["reflection", "Рефлексия"],
  ["manage", "Управление"],
] as const;

const WORK_TABS = [
  ["chat", "Чат"], ["date", "Дата"], ["orders", "Заказы"], ["longterm", "Долгосрочное"], ["manage", "Управление"],
] as const;
      ...
      {tab === "date" && (
        <div className="view">
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
            <div className="why" onClick={() => setSelectedDate(addDaysStr(selectedDate, -1))}>← Вчера</div>
            <h1 style={{ margin: 0 }}>
              {selectedDate === todayValue ? "Сегодня" : selectedDate === tomorrowValue ? "Завтра" : selectedDate === yesterdayValue ? "Вчера" : selectedDate}
            </h1>
            <div className="why" onClick={() => setSelectedDate(addDaysStr(selectedDate, 1))}>Завтра →</div>
