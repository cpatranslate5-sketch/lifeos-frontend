...
function MainApp({ profile, onSwitchFolder }: { profile: string; onSwitchFolder: () => void }) {
  ...
  async function refresh() {
    const [ents, lp] = await Promise.all([
      fetchEntities(undefined, space === "general" ? undefined : space, profile),
      space === "life" ? getLifephase(profile) : Promise.resolve(null),
    ]);
    if (space === "life") {
  ...
  return (
    <div className="app">
      <div className="folder-bar">
        <span>{FOLDER_LABELS[profile as Folder] || profile}</span>
      ...
      {tab === "movies" && <ShelfTab title="Кино" placeholder="Название фильма…" type="movie" items={movies} onChanged={refresh} profile={profile} />}
      {tab === "shows" && <ShelfTab title="Сериалы" placeholder="Название сериала…" type="show" items={shows} onChanged={refresh} profile={profile} />}
      {tab === "books" && <ShelfTab title="Книги" placeholder="Название книги…" type="book" items={books} onChanged={refresh} profile={profile} />}
      ...
      {showLPModal && <LifephaseModal current={lifephase} onClose={() => setShowLPModal(false)} onSaved={refresh} profile={profile} />}
    </div>
  );
}
...
export default function App() {
  ...
  if (!folder) {
    return <ProfileGate onPick={(f) => { saveFolder(f); setFolder(f); }} />;
  }
  ...
  if (folder === "pd") {
    return <Diary onSwitchFolder={switchFolder} />;
  }

  return <MainApp profile={folder} onSwitchFolder={switchFolder} />;
}
