  ...
  return (
    <div className="app">
      {decorCovers.length > 0 && (
        <>
          <div className="side-decor-grid left">
            {decorCovers.map((c, i) => <img key={`l${i}`} src={entityCoverUrl(c)} alt="" />)}
          </div>
          <div className="side-decor-grid right">
            {[...decorCovers].reverse().map((c, i) => <img key={`r${i}`} src={entityCoverUrl(c)} alt="" />)}
          </div>
        </>
      )}
