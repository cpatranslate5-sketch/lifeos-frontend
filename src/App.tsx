...
function MainApp({ profile, onSwitchFolder }: { profile: string; onSwitchFolder: () => void }) {
  ...
  const [decorCover, setDecorCover] = useState<string | null>(null);

  useEffect(() => {
    fetchEntities(undefined, "life", profile).then(life => {
      const candidates = life.filter(e =>
        ["movie", "show", "book", "game"].includes(e.type) &&
        e.attributes?.status === "in_progress" && e.attributes?.cover_path
      );
      if (candidates.length === 0) { setDecorCover(null); return; }
      const pick = candidates[Math.floor(Math.random() * candidates.length)];
      setDecorCover(pick.attributes.cover_path);
    });
  }, [profile]);
  ...
  return (
    <div className="app">
      {decorCover && (
        <>
          <div className="side-decor left" style={{ backgroundImage: `url(${entityCoverUrl(decorCover)})` }} />
          <div className="side-decor right" style={{ backgroundImage: `url(${entityCoverUrl(decorCover)})` }} />
        </>
      )}
