...
const WORK_TABS = [
  ["chat", "Чат"], ["date", "Задачи"], ["orders", "Заказы"], ["longterm", "Долгосрочное"], ["manage", "Управление"],
] as const;
...
function relativeDayLabel(dateStr: string): string {
  ...
  if (dateStr === addDaysStr(today, 2)) return "Послезавтра";
  if (dateStr === addDaysStr(today, -2)) return "Позавчера";
  return dateStr;
}
...
function itemsForDate(entities: Entity[], dateStr: string, isToday: boolean): Entity[] {
  return entities.filter(e => {
    ...
    if (e.type === "anniversary") {
      ...
      return Number(e.attributes?.month) === m && Number(e.attributes?.day) === d;
    }
    return false;
  });
}
...
function MainApp({ profile, onSwitchFolder }: { profile: string; onSwitchFolder: () => void }) {
  const [space, setSpace] = useState<"life" | "work" | "general">("general");
  ...
  useEffect(() => {
    if (liteMode) { setDecorCovers([]); return; }
  ...
  const sportEvents = entities.filter(e => e.type === "event" && e.attributes?.category === "sport");

  return (
    <div className="app-shell">
      {!liteMode && <SpiderDrop entities={entities} />}
    ...
    <div className="app">
      <div className="folder-bar">
        <span className="why folder-home-link" onClick={onSwitchFolder}>
          На главную
        </span>
      </div>
      <div className="space-switch">
        ...
        <div className="theme-toggle" onClick={() => setLiteMode(!liteMode)} title="Лёгкая версия — без фото и анимаций">
          {liteMode ? "🐢" : "⚡"}
        </div>
        <NotificationBell profile={profile} refreshTrigger={notifTick} onNavigate={(date) => {
          setSpace("general");
          setTab("date");
      ...
      {tab === "date" && (
        <div className="view">
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
            <div className="why" onClick={() => setSelectedDate(addDaysStr(selectedDate, -1))}>← {relativeDayLabel(addDaysStr(selectedDate, -1))}</div>
            <span className="why date-nav-current">{relativeDayLabel(selectedDate)}</span>
            <div className="why" onClick={() => setSelectedDate(addDaysStr(selectedDate, 1))}>{relativeDayLabel(addDaysStr(selectedDate, 1))} →</div>
            <span className="edit-link" onClick={() => setShowDatePicker(!showDatePicker)}>К дате</span>
            {selectedDate !== todayValue && <span className="edit-link" onClick={() => setSelectedDate(todayValue)}>К сегодня</span>}
          </div>
          {showDatePicker && (
            <div className="mini-calendar-inline-wrap">
              <MiniCalendar initialDate={selectedDate} onSelect={(d) => { setSelectedDate(d); setShowDatePicker(false); }} />
            </div>
          )}
          ...
          <QuickTaskAdd date={selectedDate} onAdded={refresh} space={space} profile={profile} />
